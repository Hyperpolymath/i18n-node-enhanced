...
This directory contains utilities for managing and validating i18n
translation files.
...
Validates translation files for common issues like: - Malformed JSON -
Duplicate keys (case variations) - Suspicious values (excessive
placeholders, typos) - Invalid plural forms - Security issues (XSS,
injection) - Formatting inconsistencies - Untranslated strings
...
*Options:* - `+--dir <path>+` - Directory containing locale files
(default: ./locales) - `+--reference <locale>+` - Reference locale to
compare against (default: en) - `+--fix+` - Auto-fix issues where
possible - `+--format+` - Format JSON files - `+--strict+` - Strict mode
- exit with error on warnings - `+--json+` - Output results as JSON
...
*Options:* - `+--dir <path>+` - Directory containing locale files
(default: ./locales) - `+--reference <locale>+` - Reference locale
(default: en) - `+--format <type>+` - Output format:
text|json|csv|markdown (default: text) - `+--output <file>+` - Output
file (default: stdout) - `+--locale <locale>+` - Check specific locale
only - `+--create-missing+` - Create missing translation files with TODO
markers
...
Run validation regularly: - Before every commit (pre-commit hook) - In
CI/CD pipeline - Before releases
